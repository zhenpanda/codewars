...
var towns = ["Rome", "London", "Paris", "NY", "Vancouver", "Sydney", "Bangkok", "Tokyo",
               "Beijing", "Lima", "Montevideo", "Caracas", "Madrid", "Berlin"]

// mean("London", data), 51.19(9999999999996)
// variance("London", data), 57.42(833333333374)

// find the town in question
function find(town, strng) {
  // split by town name
  var ary = strng.split("\n");
  // find the town from input
  for (var i = 0; i < ary.length; i++) {
    var townName = ary[i].split(":");
    //console.log(townName);
    if (townName[0] == town) {
      //console.log("found town", townName);
      var months = townName[1].split(",");
      //console.log(months);
      var rainNums = [];
      for (var m = 0; m < months.length; m++) {
        // parse the damn float
        rainNums.push( parseFloat(months[m].substring(4)) );
      }
      return rainNums;
    }
  }
...
function mean(town, strng) {
    var res = find(town,strng);
    //console.log(res);
    if (res) {
      var sum = res.reduce(function(a, b) { return a + b; });
